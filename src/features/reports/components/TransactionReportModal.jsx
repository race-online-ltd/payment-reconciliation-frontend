...
export default function TransactionReportModal({
  ...
  const [channels,   setChannels]   = useState([]);
  const [allWallets, setAllWallets] = useState([]); // ← fetch ALL wallets once, filter on frontend
  ...
  // ─── fetch ALL wallets once on mount ─────────────────────────────
  // Backend returns all wallets with payment_channel_id on each row.
  // We filter by payment_channel_id on the frontend when a channel is selected.
  useEffect(() => {
    api.get("/wallets").then((res) => {
      const data = res.data.data ?? [];
      setAllWallets(data); // keep raw objects so we can filter by payment_channel_id
    });
  }, []);
          ...
          <Form>
            ...
            >
              <Stack spacing={2}>
                ...
                {/* ── Wallet dropdown ──────────────────────────────────────────
                    key={values.channel_id} remounts the component on channel change
                    so fetchOptions re-runs and shows only wallets for that channel.
                    Filtering uses payment_channel_id (the actual field from the API). ── */}
                <SelectDropdownSingle
                  key={values.channel_id}
                  name="wallet_id"
                  placeholder="Select Wallet"
                  fetchOptions={async () => {
                    ...
                    return allWallets
                      .filter(
                        (w) => String(w.payment_channel_id) === String(values.channel_id)
                      )
